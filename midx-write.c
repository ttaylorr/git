...
static void compute_sorted_entries_compact(struct write_midx_context *ctx)
{
	uint32_t from, to;
	uint32_t i;
	...
	from = ctx->compact_from->num_objects_in_base;
	to = ctx->compact_to->num_objects + ctx->compact_to->num_objects_in_base;

	CALLOC_ARRAY(ctx->entries, to - from);
	ctx->entries_nr = 0;

	for (i = from; i < to; i++) {
		if (nth_midxed_pack_midx_entry(ctx->compact_to,
					       &ctx->entries[ctx->entries_nr++],
					       i) < 0)
			die(_("failed to get MIDX entry: %"PRIu32), i);
	}

	/*
	 * At this point, ctx->entries contains the set of objects to
	 * write into the compacted MIDX. This is equivalent to the
	 * union of objects in the MIDX layers between 'from' and 'to'.
	 *
	 * Since we added these entires in the order they appear in the
	 * original MIDX chain, the list is only partially sorted
	 * (lexicographically within each MIDX layer)->.
	 *
	 * As a result, if 'from' and 'to' are distinct layers, then we
	 * must sort ctx->entires to ensure the entire array is sorted.
	 */
...
static int write_midx_internal(struct write_midx_opts *opts)
{
	...
	if (ctx.compact)
		compute_sorted_entries_compact(&ctx);
	else
