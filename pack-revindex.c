...
static int midx_pack_order_cmp(const void *va, const void *vb)
{
	...
	int key_preferred = key->pack == key->preferred_pack;
	int versus_preferred = versus_pack == key->preferred_pack;
				...
				uint32_t *pos)
{
	...
	/*
	 * The preferred pack sorts first, so determine its identifier
	 * by looking at the first object in pseudo-pack order.
	 *
	 * Note that if no --preferred-pack is explicitly given when
	 * writing a multi-pack index, then whichever pack has the
	 * lowest identifier implicitly is preferred (and includes all
	 * its objects, since ties are broken first by pack identifier).
	 */
