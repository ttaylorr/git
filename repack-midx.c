...
S
	if (opts->names->nr) {
		...
		ALLOC_GROW_BY(steps, steps_nr, 1, steps_alloc);
		step = &steps[steps_nr++];
	...
	for (; m; m = m->base_midx) {
		...
		steps[steps_nr].type = MIDX_COMPACTION_STEP_COPY;
		steps[steps_nr].u.copy = m;
	}
...
static int write_midx_incremental(struct repack_write_midx_opts *opts)
{
	...
	if (opts->geometry->split_factor) {
		...
		if (repack_make_midx_append_plan(opts, &steps, &steps_nr) < 0) {
			ret = error(_("unable to generate append plan"));
			goto done;
		}
	}
