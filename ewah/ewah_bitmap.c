...
/*
 * Compute `ewah_i | ewah_j` directly into `out`, walking both inputs
 * via run-length-word iterators rather than materializing a dense
 * intermediate. The output is built in canonical EWAH order so it can
 * be interned alongside any other `bitmap_to_ewah()`-shaped pool
 * entry.
 *
 * Modeled on `ewah_xor()` above; the only structural difference is
 * the per-word combiner (OR vs XOR) and the running-bit logic, since
 * a run of 1s on the predator-side absorbs the prey unconditionally
 * for OR.
 */
void ewah_or(
	...
	struct ewah_bitmap *out)
{
	...
	while (rlwit_word_size(&rlw_i) > 0 && rlwit_word_size(&rlw_j) > 0) {
		while (rlw_i.rlw.running_len > 0 || rlw_j.rlw.running_len > 0) {
			...
			if (predator->rlw.running_bit) {
				/*
				 * Predator runs 1s. The OR is 1s for the
				 * full predator->running_len; advance the
				 * prey by the same number of words.
				 */
				ewah_add_empty_words(out, 1,
					predator->rlw.running_len);
				rlwit_discard_first_words(prey,
					predator->rlw.running_len);
			} else {
				/*
				 * Predator runs 0s. The OR equals the prey's
				 * bits over the predator's running_len words;
				 * top up any tail with zeros.
				 */
	...
	/*
	 * Whichever operand still has words left is OR'd with zero (the
	 * other operand is implicitly zero past its end), so it passes
	 * through unchanged.
	 */
