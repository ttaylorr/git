...
static int read_midx_disjoint_packs(const char *object_dir)
{
	struct multi_pack_index *midx = NULL;
...
{
	if (!(argc == 2 || argc == 3))
		usage("read-midx [--show-objects|--checksum|--preferred-pack|--show-disjoint] <object-dir>");
	...
	else if (!strcmp(argv[1], "--show-disjoint"))
		return read_midx_disjoint_packs(argv[2]);
