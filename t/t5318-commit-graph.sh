...
test_expect_success 'git fsck (suppresses progress output when asked)' '
	GIT_PROGRESS_DELAY=0 git -C full fsck --progress 2>err &&
	grep "Verifying commits in commit graph" err &&
	GIT_PROGRESS_DELAY=0 git -C full fsck --no-progress 2>err &&
	! grep "Verifying commits in commit graph" err
'
