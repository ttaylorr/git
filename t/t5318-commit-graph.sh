...
graph_expect_commits() {
	...
	test_i18ngrep "num_commits: $1" got
}

test_expect_success 'ignores non-commit OIDs to --input=stdin-commits with --no-verify-oids' '
	...
	echo $ZERO_OID >bad &&
	test_expect_code 1 git commit-graph write --stdin-commits \
		--no-verify-oids <bad 2>err &&
	test_i18ngrep "no such object: $ZERO_OID" err &&
	graph_expect_commits 2 &&
	# update with valid commit OID, ignore tree OID
	git rev-parse HEAD HEAD^{tree} >in &&
	git commit-graph write --stdin-commits --no-verify-oids <in &&
