...
# test_pack_objects_reused__thin <pack-reused> <packs-reused>
...
test_expect_success 'can retain delta from uninteresting base (cross pack)' '
	...
	git for-each-ref --format="%(objectname)" >in &&
	test_pack_objects_reused_thin $objects_nr $packs_nr <in
'
...
test_expect_success 'converts OFS_DELTA to REF_DELTA when possible' '
	...
	(
		...
		cat <<-EOF | test_pack_objects_reused_thin 3 1
		$delta
		^$base
		EOF
	)
