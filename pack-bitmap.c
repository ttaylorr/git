...
static inline uint64_t read_be64(const unsigned char *buffer, size_t *pos)
{
	uint64_t result = get_be64(buffer + *pos);
	(*pos) += sizeof(result);
	return result;
}
...
static inline const void *xor_position_pos(struct bitmap_index *bitmap_git,
					   uint32_t pos)
{
	return bitmap_offset_pos(bitmap_git, pos) + sizeof(uint32_t);
}
