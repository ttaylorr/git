...
struct bitmap_index {
	...
	/*
	 * Whether or not the last performed walk might have had any
	 * bits removed from 'result' due to object filtering.
	 */
	int filtered;
					 ...
					 int filter_provided_objects)
{
	...
	bitmap_git->filtered = !revs->filter || revs->filter.choice != LOFC_DISABLED;
					...
					int multi_pack_reuse)
{
	...
	if (multi_pack_reuse) {
		...
		packs[packs_nr].to_ref_delta =
			bitmap_word_alloc(pack->num_objects / BITS_IN_EWORD);
