...
bundle.<id>.filter::
	This string value defines the filter specification that was used when
	generating this bundle. Clients may optionally use this to inform which
	bundle(s) they are interested in based on their own `--filter` setting.
	For more details on filter specifications, see the `--filter` option in
