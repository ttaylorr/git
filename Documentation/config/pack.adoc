...
pack.reuseExternalDeltas::
	When true, converts offset deltas to reference deltas when using
	a multi-pack reachability bitmap during reuse for
	delta/base-pairs that are recorded from different packs in the
	MIDX. When false, offset deltas whose base is recorded from a
	different pack are not reused verbatim. Defaults to false.
...
pack.useMultiPackForwardIndex::
	When true, Git will generate a forward-index from MIDX
	lexicographic order to pseudo-pack order. This can speed up
	verbatim pack-reuse at the expense of additional memory
	consumption.

pack.writeMultiPackFowardIndex::
	When true, Git will write a forward-index `FIDX` chunk to map
	MIDX entries from lexicographic order to pseudo-pack order. Git
	will use this when available (provided MIDX forward indexes are
	enabled via `pack.useMultiPackForwardIndex`) instead of
