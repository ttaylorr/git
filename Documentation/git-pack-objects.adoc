...
+
When used with `--delta-islands`, island marks are propagated from ref
tips down to excluded-closed pack boundaries.
+
Incompatible with `--revs`, or options that imply `--revs` (such as
`--all`), with the exception of `--unpacked`, which is compatible.
`--delta-islands` may only be used with `--stdin-packs=follow`.
